...
def _gather_samples(roidb, roi_inds, rels, num_classes):
    ...
    for i, rel in enumerate(rels):
        ...
        rels[i] = [roi_ind_map[rel[0]],  roi_ind_map[rel[1]] ] + list( rel[2:] )
...
def _sample_graph(roidb, num_fg_rois, num_rois, num_neg_rels=128):
    ...
    # kind of remake the fg_gt_ind_assignments
    gt_to_fg_roi_inds = {}
    all_fg_roi_inds = []
    ...
    for rel in gt_rels:
        for sub_i in gt_to_fg_roi_inds[rel[0]]:
            for obj_i in gt_to_fg_roi_inds[rel[1]]:
                ...
                pos_rels.append([sub_i, obj_i] + list(rel[2:]))
    ...
    if len(pos_rels) > 0:
        ...
        _, indices = np.unique(["{} {}".format(pos_rel[0], pos_rel[1]) for pos_rel in pos_rels], return_index=True)
        pos_rels = np.array(pos_rels)[indices, :]
        #pos_rels = np.array(pos_rels)[:, :]
    ...
    for i in roi_inds:
        for j in roi_inds:
            if i != j and [i, j] not in rels_inds:
                sample_rels.append([i,j]+[0]*(pos_rels.shape[1]-2)) #TOCHECK
