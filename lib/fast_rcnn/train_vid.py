...
class Trainer(object):
    ...
    def get_data_runner(self, sess, data_layer):
        ...
        input_pls = {
            ...
            'oids': tf.placeholder(dtype=tf.int32, shape=[None,1]),
            'seq_name': tf.placeholder(dtype=tf.string),
    ...
    def train_model(self, sess, max_iters):
        ...
        data_layer = RoIDataLayerVid(self.imdb, self.bbox_means, self.bbox_stds)
        # a multi-process data runner
        data_runner = self.get_data_runner(sess, data_layer)
        data_layer._get_next_minibatch(0)
