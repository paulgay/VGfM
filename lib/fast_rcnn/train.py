...
class Trainer(object):
    ...
    def get_data_runner_multi(self, sess, data_layer):

        input_pls = {
            'ims': tf.placeholder(dtype=tf.float32, shape=[None, None, None, 3]),
            'rois': tf.placeholder(dtype=tf.float32, shape=[None, 5]), #they seem to be duplicated
            'rel_rois': tf.placeholder(dtype=tf.float32, shape=[None, 5]),
            'labels': tf.placeholder(dtype=tf.int32, shape=[None]),
            'relations': tf.placeholder(dtype=tf.int32, shape=[None, 2]),
            'predicates': tf.placeholder(dtype=tf.float32, shape=[None, self.imdb.num_predicates]),
            ...
            'db_idx': tf.placeholder(dtype=tf.int32, shape=[None])
        }
    ...
    def get_data_runnerbw(self, sess, data_layer):
    ...
    def train_model(self, sess, max_iters):
        ...
        if hasattr(self.imdb,"multi_label"):
            data_runner =  self.get_data_runner_multi(sess, data_layer)
        else:
        ...
        for i in range(200):
            print 'batch',i
            batch = data_layer.next_batch()
            q = batch['quadric_rois']
            for j in range(len(batch['relations'])): 
                (o1,o2) = batch['relations'][j]
                if q[o1,1] < q[o2,1]:
                    pred = np.vstack((pred,2))
                else:
                    pred = np.vstack((pred,1))
                gt = np.vstack((gt,batch['predicates'][j]))
        np.mean(gt==pred)
        import pdb; pdb.set_trace()
        """
         
        #lll = data_layer._get_next_minibatch([ 2499, 11705,  5198, 10613, 10988,  1459,  6178,  7390])
        ...
        if self.pretrained_model is not None:
            ...
            if self.pretrained_model.endswith('.npy'):
                ...
                not_restored = ['geo_fc2_obj/biases:0', 'geo_fc1_obj/biases:0', 'geo_w_fc/biases:0', 'geo_fc2_obj/weights:0', 'geo_fc1_obj/weights:0', 'geo_w_fc/weights:0']
                restore_var = [v for v in tf.trainable_variables() if v.name not in not_restored]
                #import pdb; pdb.set_trace()
                #saver_loc = tf.train.Saver(restore_var, max_to_keep=None)
                #saver_loc.restore(sess, self.pretrained_model)
                self.saver.restore(sess, self.pretrained_model)
                iter_n = int(os.path.basename(self.pretrained_model).split('.')[0].split('_')[1])
            else:
        ...
        for iter in range(iter_n+2,max_iters):
            # learning rate
            iter_timer.tic()
            ...
            #print(sess.run(self.net.rois, feed_dict=feed_dict))
            #print(sess.run(self.net.data['bbox_targets'], feed_dict=feed_dict))
            #vgg_out=sess.run(self.net.get_output('vgg_out'),feed_dict=feed_dict)
            #conv_out=sess.run(self.net.get_output('conv_out'),feed_dict=feed_dict)
            #np.savetxt('idx_224_14_448_640.txt',conv_out)
            timer.tic()
