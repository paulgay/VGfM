...
class vg_hdf5_multi(imdb):
    def __init__(self, roidb_file, dict_file, imdb_file, rpndb_file, data_dir, split, num_im):
        ...
        self.roidb_idx_to_imdbidx = self.rpn_h5['im_to_imdb_idx' ][split_mask]# no split_mask for that
        ...
        self._relation_predicates = self.roi_h5['predicates_all']
    ...
    def make_im2seq(self):
      self.seq2im = self.rpn_h5['seq_to_im_idx']
      self.seq2im_mask = {}
      ...
      for s in range(len(seq_names)):
        ...
        for i in range(num_im[s]):    
          i_idx = first_im + i 
          self.im2seq[i_idx] = seq_name
      import pdb;pdb.set_trace()
      for i in range(len(self._image_index)):
        self.im2seq_sp[i] = self.im2seq[self._image_index[i]]
      for (i,s) in self.im2seq.items():
        if s not in self.seq2im:
          self.seq2im[s] = []
        self.seq2im[s].append(i)
      self.seq2im_sp = {}
      for (i,s) in self.im2seq_sp.items():
        if s not in self.seq2im_sp:
          self.seq2im_sp[s] = []
        self.seq2im_sp[s].append(i)      
      self.seqs_sp = self.seq2im_sp.keys() 
        ...
        for i in range(self.num_images):
            ...
            if self.im_to_first_rel[i] >= 0: # if image has relations
                ...
                for j, p in enumerate(predicates):
                    gt_relations.append([obj_idx[j][0], obj_idx[j][1]] + list(p))
