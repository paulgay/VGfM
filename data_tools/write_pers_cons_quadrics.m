clear all; close all;
load('/home/gay/exp_icip_2018/pers_calibrated/psfmo_persp_c_constraints/pers_all.mat')
addpath('/home/gay/exp_icip_2018/pers_calibrated/psfmo_persp_c_constraints/functions/');
seqdir = '/ssd_disk/gay/scenegraph/sparse_seqs/';
fid = fopen('/ssd_disk/gay/scenegraph/sparse_seqs/pers_cons_bbx.txt','w');
for ii =1:length(seqs)
  trackfile=[seqdir '/' seqs(subset(ii)).name];
  trackfile
  seq_name = strrep( seqs(subset(ii)).name,'.seq','');
  ...
  no = length(ojs);
  nf = length(Imm);
  ell=quadric2ellipsoide(allresults{ii}.rec1_cen);
